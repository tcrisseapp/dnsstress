...
func (r *Resolver) exchange() error {
	switch r.dnsProtocol {
	case DNS:
		return r.exchangeDNS()
	case DNSOverHTTPS:
		return r.exchangeDNSoverHTTPS()
	default:
		return fmt.Errorf("unknown dns protocol: %s", r.dnsProtocol)
	}
}

func (r *Resolver) exchangeDNS() error {
	msg := new(dns.Msg).SetQuestion(r.domain, dns.TypeA)
	resp, err := dns.ExchangeContext(r.ctx, msg, r.server)
	if err != nil {
		return err
	}

	atomic.AddInt64(&r.sent, 1)
	atomic.AddInt64(&r.bytesReceived, int64(resp.Len()))
	return nil
}

func (r *Resolver) exchangeDNSoverHTTPS() error {
	msg := new(dns.Msg).SetQuestion(r.domain, dns.TypeA)
	...
	url := r.server + "?dns=" + base64.RawURLEncoding.EncodeToString(buf)
	req, err := http.NewRequestWithContext(r.ctx, "GET", url, http.NoBody)
	if err != nil {
		return err
	}
	...
	atomic.AddInt64(&r.sent, 1)
	atomic.AddInt64(&r.bytesReceived, int64(res.ContentLength))
	return nil
}
...
func (r *Resolver) submitStats() {
	...
	bytesReceived := atomic.SwapInt64(&r.bytesReceived, 0)
	...
	_ = r.statsdReporter.Count("npm.testing.bytes_received", bytesReceived, tags, 1)

	// Update totals
	totalSent := atomic.AddInt64(&r.totalSent, sent)
	atomic.AddInt64(&r.totalErrors, errors)
	atomic.AddInt64(&r.totalBytesReceived, bytesReceived)
